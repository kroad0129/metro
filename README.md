...
외부 API를 실제로 호출하는 테스트는 없다. `backend/test/fixtures/real/`에 실제 응답을 캡처해 두고 그걸로 검증한다. 프론트에는 실제 API를 6분간 녹화한 데이터를 모델에 그대로 통과시켜 "시간이 늘지 않는다 · 점이 뒤로 가지 않는다" 같은 약속을 단언하는 **재생 테스트**(`virtualTrain.replay.test.ts`)가 있다.
...
**시간·위치는 "가상 열차" 모델이 만든다.** 서울시 `barvlDt`는 실시간 카운트다운이 아니라 (역·방향·거리)별 고정 조회표다 — 실측으로 확인했다. 그래서 `ordkey`의 정거장 거리와 구간 진입 시각을 앵커로 클라이언트가 열차를 시뮬레이션하고, 다음 구간이 확인되기 전엔 역 직전에서 멈춰 거짓 도착을 만들지 않는다. 지연이 감지되면 "지연" 배지가 붙는다. 상세는 [`docs/superpowers/specs/2026-07-23-time-model-from-measurement.md`](docs/superpowers/specs/2026-07-23-time-model-from-measurement.md).
