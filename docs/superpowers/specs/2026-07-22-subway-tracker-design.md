...
| 급행 구분 | 언급 없음 | **표시 + 필터링 추가** | 증미역은 급행 미정차역이다. 서지 않는 열차를 "2분 후 도착"으로 표시하면 성공기준 7("실제 판단에 도움")을 정면으로 해친다. |
...
│   ├── trains.service.ts       방향 분류, 급행 필터, 캐시 조율, stale 처리
...
**급행 필터링**: 선택한 역에 정차하지 않는 급행 열차는 `trains` 배열에서 제외한다. `trainType`은 응답에 남겨 두어, 나중에 "통과 예정"으로 표시 방식을 바꿀 때 백엔드를 수정하지 않아도 되게 한다.
...
- `isExpressStop`은 급행 필터링의 유일한 근거다. 값이 틀리면 열차가 잘못 걸러지므로 공식 자료로 대조한다(2절 6번).
...
급행 뱃지는 **선택한 역에 정차하는 급행**일 때만 붙인다. 정차하지 않는 급행은 백엔드에서 이미 제외되었다.
...
| `trains.service` | 방향 분류, 급행 필터, stale 반환, 캐시 히트/미스 | 가능 (외부 목킹) |
...
수동 확인 항목: 증미역 기준 양방향 표시, 급행 제외 동작, 새로고침 쿨다운, 갱신 실패 시 stale 배너, 앱 재진입 시 역 유지, 스마트폰 세로 화면 가독성.
...
7. 증미역에 서지 않는 급행이 도착 예정으로 표시되지 않는다.
